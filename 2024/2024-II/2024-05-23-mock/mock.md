...
то есть, это свойство явно выражено в коде.


### 3.2. И швец и жнец
Тест проверяет, что функция `extract_params`
извлекает параметры, для всех возможных их комбинаций
обёрток `Parameterized` и `Sliced`
(по спецификации нет варианта `Sliced(Parameterized(a))`).

Все варианты (включая отсутствие обёрток) проверяются,
то есть, это свойство явно выражено в коде.


### 3.3. Великий `Combinable`
Тест проверяет, что функция `apply` добавляет параметры к элементам,
насквозь проходя через все обёртки и не меняя структуру этих обёрток.

Для прохождения сквозь обёртки уже есть `dest_map`,
так что достаточно проверить, что `apply` - её частный случай.

Тестируемое свойство явно выражено в коде.
Тест очень похож на доказательство по индукции.
Сначала тестируется работа на тривиальных случаях из одного входного элемента.
Затем результат для набора элементов сравнивается с набором результатов
для отдельных элементов.


### 3.4. Идём в глубину
Функция `dest_map` применяет функцию к набору элементов,
следуя определённой спецификации, которая зашифрована в слове "dest".

Код теста, фактически, является этой спецификацией.
То есть, тестируемое свойство явно выражено в коде.


### 3.5. Родной брат
Функция `dest_map_reduce` - родственник предыдущей.
Она не только применяет другую функцию, но и собирает результат
по всем элементам.

Снова код теста является спецификацией
и тем самым явно выражает тестируемое свойство.


### Выводы
Хорошие тесты похожи на математические доказательства.
Они перебирают все пункты требований (необходимые условия).
Ещё они могут опираться сами на себя по принципу математической индукции.
Достаточными они, как правило, не являются, но уже дают много.


## 4. Моки
В реальном коде большинство функций используются для осей тензоров,
которые и `Combinable`, и `Parameterizeable`, и `Sliceable`,
и ещё много чего.

Старые версии тестов активно использовали самодельные моки.
В новых версиях, благодаря ликвидации лишних требований к объектам,
стало возможным использовать в качестве моков встроенные типы.

В случае с царь-формулой в качестве мока выступает простой тестовый набор тензоров,
который используется вместо настоящего с десятком параметров.
